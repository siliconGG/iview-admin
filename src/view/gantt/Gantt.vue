...
<script>
import 'dhtmlx-gantt'
// import '../../../node_modules/dhtmlx-gantt/codebase/ext/dhtmlxgantt_tooltip.js'
import '../../../node_modules/dhtmlx-gantt/codebase/ext/dhtmlxgantt_marker.js'
import '../../../node_modules/dhtmlx-gantt/codebase/ext/dhtmlxgantt_undo.js'
...
export default {
  ...
  props: {
    tasks: {
      type: Object,
      default () {
        return {data: [], links: [], priorityList: [], ganttPersonList: []}
      }
    }
  },
  beforeCreate () {
  ...
  methods: {
    ...
    modified: function (task) {
      let that = this
      let instance = this.$ajax.create({
        // 用来将token放到header上
        headers: {'token': window.localStorage.getItem('token')}
      })
      switch (task.act_type) {
        case '活动' :
          instance.post(that.GLOBAL.serverPath + '/activity/modifyActivity', {
            actId: parseInt(task.id),
            actName: task.text,
            actShort: task.act_short,
            personId: task.person_id,
            actEngName: task.act_eng_name,
            actAddress: task.act_address,
            actMemo: task.act_memo,
            actPriority: task.priority,
            actDateFrom: task.start_date,
            actDateTo: gantt.templates.date_grid(task.end_date, task)
          ...
          break
        case '子活动' :
          instance.post(that.GLOBAL.serverPath + '/activity/modifyActivitySub', {
            actSubId: parseInt(task.id),
            actSubName: task.text,
            actSubShort: task.act_short,
            personId: task.person_id,
            duty: task.duty,
            actSubEngName: task.act_eng_name,
            actSubAddress: task.act_address,
            actSubMemo: task.act_memo,
            actSubPriority: task.priority,
            actSubDateFrom: task.start_date,
            actSubDateTo: gantt.templates.date_grid(task.end_date, task)
          }).then(function (response) {
            if (response.data.success) {
              alert('修改成功！')
            } else {
        ...
        case '任务' :
          instance.post(that.GLOBAL.serverPath + '/task/modifyTask', {
            ...
            taskName: task.text,
            taskShort: task.act_short,
            personId: task.person_id,
            duty: task.duty,
            taskEngName: task.act_eng_name,
            taskAddress: task.act_address,
            taskMemo: task.act_memo,
            taskPriority: task.priority,
            taskDateFrom: task.start_date,
            taskDateTo: gantt.templates.date_grid(task.end_date, task)
          }).then(function (response) {
            if (response.data.success) {
              alert('修改成功！')
            } else {
    ...
    $_initGanttEvents: function () {
      ...
      gantt.attachEvent('onBeforeLightbox', function (id) {
        ...
        if (task.act_type === '活动') {
          newSections = [
            ...
            {name: 'users', height: 22, map_to: 'person_id', type: 'select', options: that.tasks.ganttPersonList},
            {name: 'priority', height: 22, map_to: 'priority', type: 'select', options: that.tasks.priorityList},
            {name: 'act_short', height: 38, map_to: 'act_short', type: 'textarea', focus: true},
            {name: 'act_eng_name', height: 38, map_to: 'act_eng_name', type: 'textarea', focus: true},
            {name: 'act_memo', height: 38, map_to: 'act_memo', type: 'textarea', focus: true},
            {name: 'act_address', height: 38, map_to: 'act_address', type: 'textarea', focus: true},
            {name: 'time', type: 'time', map_to: 'auto', time_format: ['%Y', '%m', '%d']}]
        } else if (task.act_type === '子活动') {
          newSections = [{name: 'description', height: 38, map_to: 'text', type: 'textarea', focus: true},
            {name: 'users', height: 22, map_to: 'person_id', type: 'select', options: that.tasks.ganttPersonList},
            {name: 'duty', height: 22, map_to: 'duty', type: 'textarea', focus: true},
            {name: 'priority', height: 22, map_to: 'priority', type: 'select', default_value: '正常', options: that.tasks.priorityList},
            {name: 'act_short', height: 38, map_to: 'act_short', type: 'textarea', focus: true},
            {name: 'act_eng_name', height: 38, map_to: 'act_eng_name', type: 'textarea', focus: true},
            {name: 'act_memo', height: 38, map_to: 'act_memo', type: 'textarea', focus: true},
            {name: 'act_address', height: 38, map_to: 'act_address', type: 'textarea', focus: true},
            {name: 'time', type: 'time', map_to: 'auto', time_format: ['%Y', '%m', '%d']}]
        } else if (task.act_type === '任务') {
          newSections = [{name: 'description', height: 38, map_to: 'text', type: 'textarea', focus: true},
            {name: 'users', height: 22, map_to: 'person_id', type: 'select', options: that.tasks.ganttPersonList},
            {name: 'duty', height: 22, map_to: 'duty', type: 'textarea', focus: true},
            {name: 'act_type', height: 38, map_to: 'act_type', type: 'textarea', focus: true},
            {name: 'priority', height: 22, map_to: 'priority', type: 'select', default_value: '正常', options: that.tasks.priorityList},
            {name: 'act_short', height: 38, map_to: 'act_short', type: 'textarea', focus: true},
            {name: 'act_eng_name', height: 38, map_to: 'act_eng_name', type: 'textarea', focus: true},
            {name: 'act_memo', height: 38, map_to: 'act_memo', type: 'textarea', focus: true},
        ...
        gantt.config.lightbox.sections = newSections
        gantt.resetLightbox()
        return true
      })
      ...
      gantt.attachEvent('onLightboxSave', function (id, task, is_new) {
        // 将通过personId查出name 注入data
        let person_id = task.person_id
        let userName = ''
        that.tasks.ganttPersonList.forEach((person) => {
          if (person.key == person_id) {
        ...
        that.tasks.priorityList.forEach((priority) => {
          if (priority.key == task.priority) {
            prio = priority.label
          }
        })
        task.priorityName = prio
        // 调用接口修改后端数据， 若失败则回调
        that.modified(task)
        return true
      })
  ...
  mounted () {
    ...
    gantt.locale = {
      ...
      labels: {
        ...
        section_ceshi: '测试呀',
        section_act_type: '活动类型',
        section_act_short: '活动简称',
        section_act_eng_name: '活动英文名',
        section_act_memo: '活动备注',
        section_act_address: '活动地址',
        section_duty: '负责人职责',
    ...
    //   return "<b>任务:</b> "+task.text+"<br/><b>负责人:</b> " + task.users;
    // };
    ...
    gantt.config.layout = {
      ...
      cols: [
        ...
        {view: 'scrollbar', scrollable: true, id: 'scrollVer'}
      ]
    }

    // 设置左侧试图列名称
    gantt.config.columns = [{name: 'add', width: 40},
      {name: 'text', label: '活动名称', tree: true, min_width: 100, width: 100, resize: true},
      {name: 'act_type', label: '活动类型', min_width: 100, width: 100, resize: true},
      {name: 'users', label: '负责人', align: 'center', width: 100, resize: true},
      {name: 'priorityName', label: '优先级', min_width: 100, width: 100, resize: true},
    ...
    gantt.templates.task_text = function (start, end, task) {
      return '<b>名字:</b> ' + task.text + ',<b> 负责人:</b> ' + task.users
    }
    gantt.config.subscales = [
    ...
    gantt.templates.task_class = function (start, end, task) {
      if (task.start_date.getTime() + task.progress * task.duration * 86400000 < today.getTime()) {
        return 'important'
      } else {
        return ''
      }
